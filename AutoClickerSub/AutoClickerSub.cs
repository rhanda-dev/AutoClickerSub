...
public class AutoClickerSub
{
    ...
    /// <summary>
    /// store target application window info's
    /// </summary>
    public class EnumWindows
    {
        ...
        /// <returns></returns>
        public bool CalbackProc(IntPtr hWnd, IntPtr lParam)
        {
            if (hWnd != IntPtr.Zero)
            {
...
internal class NativeMethods
{
    public static (string, string) GetProceeAndFileName(int pid)
    {
        ...
        sb.Clear();
        if (GetModuleBaseName(processHandle, IntPtr.Zero, sb, isb) > 0) processname = sb.ToString();
        else processname = "";
    ...
    [DllImport("user32")]
    internal static extern bool EnumWindows(WNDENUMPROC lpEnumProc, IntPtr lParam);
...
[StructLayout(LayoutKind.Sequential)]
public struct sSENDINPUT
{
    internal uint type;
    internal InputUnion U;
    internal static int Size
    {
        get { return Marshal.SizeOf(typeof(sSENDINPUT)); }
    }
}
